...
async def pin(react: discord.RawReactionActionEvent):
    ...
    channel, reacted_msg = await general_utils.get_message_from_payload(react)
    ...
    if len(pinned_messages) == 50: await pinned_messages[0].unpin(reason = f"Unpinned through the bot.")
...
async def unpin(react: discord.RawReactionActionEvent):
    ...
    for pin_emoji in cfg.PIN_MSGS_CFG['emoji_list']:
